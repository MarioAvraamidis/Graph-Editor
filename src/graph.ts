...
export class Edge extends LineSegment
{
    ...
    constructor([v1,v2]: [Point,Point])  { super([v1,v2]); }
    ...
    removeBends(updateCrossings: boolean = true) { this._bends = []; }
}
...
export class Crossing extends Point
{
    ...
    private _legal: boolean = true;     
    private _more_than_once: boolean = false;
    ...
    // check if the given crossing has the same edges as this crossing
...
function printPointArray(points: Point[])
{
...
export class Graph {
    ...
    private self_loops: boolean = false;
    ...
    getVertexAtPosition(x: number, y: number, dist: number)
    {
    ...
    {
        let id = id1+ "-" + id2;    // as used in the constructor of class Edge
    ...
    addEdge(v1: Vertex, v2: Vertex, updateCrossings: boolean = true) {
        ...
        //check that vertices belong to the graph
        if (this._vertices.includes(v1) && this._vertices.includes(v2)) {
            ...
            else if  (extractIds(this._edges).includes(edge_id1))
                console.log("WARNING: Edge " + edge_id1 + " already exists")
            // if the graph is undirected, check reversed edge
    ...
    moveBend(b: Bend, x: number, y: number) 
    {
        b.moveTo(x,y);
        // update crossings
        if(this._effective_crossing_update)
    ...
    // remove a bend from an edge (given the vertices of the edge)
    removeBend(v: Vertex, u: Vertex)
    {
        let edge = this.getEdgeByVertices(v,u);
        if (edge)
        {
    ...
    addAllEdges()
    {
        for (const v1 of this._vertices)
            for (const v2 of this._vertices)
                this.addEdge(v1,v2,false);
        this.updateCrossings();
    }

    // place all the vertices in a circle with center (x0,y0) and radius r
    makeCircle(x0=0,y0=0,r=10)
    {
        this._vertices.forEach((vertex, index) => {
            const angle = (index / this._vertices.length) * 2 * Math.PI;
            // vertex.x = x0 + Math.cos(angle) * r;
            // vertex.y = y0 + Math.sin(angle) * r;
            // vertex.moveTo(x0 + Math.cos(angle) * r, y0 + Math.sin(angle) * r)
    ...
    extendEdge(vertex: Vertex, x: number, y: number)
    {
    ...
    extendEdgeToVertex(vertex: Vertex, newVertex: Vertex)
    {
        ...
        if (vertex.temporary)
        {
            ...
            if (this.getEdgeByVertices(neighbor,newVertex)) { console.log("Edge already exists"); return; }
            // check self loops
            if (neighbor === newVertex) { console.log("Self-loops not allowed"); return; }
            ...
            edge?.addBend(vertex.x,vertex.y,false);   // the previous temporary vertex now becomes bend
    ...
    addEdgeAdvanced(u: Vertex, v: Vertex)
    {
        if (!v.temporary)
        ...
        // check self loops
        if (uNeighbor === vNeighbor) { console.log("Self-loops not allowed"); return; }
        // check if an edge already exists
        if (this.getEdgeByVertices(uNeighbor,vNeighbor)) { console.log("Edge already exists"); return; }
        ...
        edge?.addBends(vEdge!.bends.reverse());
        // remove temporary vertex from vertices
        this.deleteVertex(u);
        this.deleteVertex(v);
        // update crossings
